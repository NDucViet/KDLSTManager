document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("items-total").innerText =
    total.toLocaleString("vi-VN") + " ₫";
  document.getElementById("total-price").innerText =
    (total + total * 0.1).toLocaleString("vi-VN") + " ₫"; // Assuming 5000&#8363; is the shipping cost
});
...
function confirmCheckOut() {
  ...
  // Đặt currentDate về 00:00:00 để so sánh chính xác hơn
  currentDate.setHours(0, 0, 0, 0);

  // Kiểm tra nếu ngày để trống
  if (!dateInput) {
  ...
  if (selectedDate < currentDate) {
    alert("Ngày chọn không thể là ngày trong quá khứ!");
    return false; // Ngăn form submit
  }
